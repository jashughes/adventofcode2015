input = [parse.(Int, x) for x in split.(readlines("02.txt"), "x")]

function wrap_needs(l, w, h)
    big_bit = (2 * l * w) + (2 * w * h) + (2 * h * l)
    little_bit = prod(sort([l, w, h])[1:2])
    big_bit + little_bit
end

function ribbon_needs(l, w, h)
    bow_bit = l * w * h
    perimeter_bit = 2 * sum(sort([l, w, h])[1:2])
    bow_bit + perimeter_bit
end

println("Part 1: ", sum(wrap_needs(x...) for x in input))
println("Part 2: ", sum(ribbon_needs(x...) for x in input))
...
the feet of ribbon required for the perfect bow is equal to the 
cubic feet of volume of the present. Don't ask how they tie the 

input = split(read("03.txt", String), "")
...
function deliver(input)
    ...
    for i in input
        pos = move(i, pos)
        route[pos] = get(route, pos, 0) + 1
    end
    route
end

function robo_deliver(input)
    d1 = input[1:2:end]
    d2 = input[2:2:end]
    union(keys(deliver(d1)), keys(deliver(d2)))
end

println("Part 1: ", length(deliver(input)))
println("Part 2: ", length(robo_deliver(input)))
